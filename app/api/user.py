...
def register(data: dict):
    ...
    refresh_expiration_time = 24 # 1 day
...
def login(data: dict):
    ...
    try:
        ...
        print(session.query(User).filter(User.username == user.username).first(), user.email)
        if user.username:
            if session.query(User).filter(User.username == user.username).first() is None:
                print("here")
                raise HTTPException(status_code=400, detail="Username does not exist")
        if user.email:
            if session.query(User).filter(User.email == user.email).first() is None:
...
def refresh_token(request: Request):
    ...
    try:
        ...
        if refresh_token_payload:
            ...
            token = Token(token=refresh_token, user_id=user_id)
